...
export async function POST(req: NextRequest) {
  try {
    ...
    const up = await fetch(`${base()}/api/wms/stock/adjust-simple`, { method: "POST", headers: hdr(req), body: JSON.stringify(body), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF inventario/ajuste POST]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
