...
export async function POST(req: NextRequest, { params }: { params: { chatId: string } }) {
  try {
    ...
    const up = await fetch(`${base()}/api/inbox/${encodeURIComponent(params.chatId)}/ml-question/answer`, { method: "POST", headers: hdr(req), body: JSON.stringify(body), cache: "no-store" });
