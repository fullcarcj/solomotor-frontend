...
export async function GET(req: NextRequest) {
  ...
  try {
    const up = await fetch(`${base()}/api/stats/mercadolibre?${p}`, { headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF ml/stats]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
