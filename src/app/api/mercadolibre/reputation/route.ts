...
export async function GET(req: NextRequest) {
  try {
    const up = await fetch(`${base()}/api/ml/reputation`, { headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF ml/reputation]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
