...
const FWD = ["status", "local_status", "q", "zero_stock", "ml_user_id", "limit", "offset"] as const;
export async function GET(req: NextRequest) {
  ...
  try {
    const up = await fetch(`${base()}/api/ml/publications?${p}`, { headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF ml/publications]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
