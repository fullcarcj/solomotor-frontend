...
export async function GET(req: NextRequest) {
  ...
  for (const k of ["limit", "offset"] as const) { const v = req.nextUrl.searchParams.get(k); if (v) p.set(k, v); }
  if (!p.has("limit")) p.set("limit", "50");
  try {
    const up = await fetch(`${base()}/api/ml/listings/unmapped?${p}`, { headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF ml/listings/unmapped]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
