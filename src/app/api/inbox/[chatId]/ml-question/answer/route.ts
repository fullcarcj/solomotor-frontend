...
/** BFF → POST /api/inbox/:chatId/ml-question/answer — body: { answer_text, answered_by } */
export async function POST(req: NextRequest, ctx: RouteCtx) {
  const { chatId } = await ctx.params;
  const body: unknown = await req.json().catch(() => ({}));
  const targetUrl = `${base()}/api/inbox/${encodeURIComponent(chatId)}/ml-question/answer`;
  ...
  try {
    ...
    console.error("[BFF inbox/ml-question/answer]", e);
