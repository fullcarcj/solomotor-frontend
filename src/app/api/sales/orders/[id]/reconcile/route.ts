...
/** BFF proxy → backend POST /api/sales/orders/:id/reconcile */
export async function POST(req: NextRequest, ctx: Ctx) {
  const { id } = await ctx.params;
  const url = `${receiverBase()}/api/sales/orders/${encodeURIComponent(id)}/reconcile`;
  ...
  try {
    ...
    if (up.status >= 500) {
      console.error("[BFF sales/orders/reconcile POST]", up.status, url);
    ...
    console.error("[BFF sales/orders/reconcile POST] red:", e);
