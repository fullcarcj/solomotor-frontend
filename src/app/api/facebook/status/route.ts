...
export async function GET(req: NextRequest) {
  try {
    const up = await fetch(`${base()}/api/facebook/status`, { headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF facebook/status]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
