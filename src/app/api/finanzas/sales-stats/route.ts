...
const FORWARD_KEYS = ["period", "from", "to", "source", "seller"] as const;
...
export async function GET(req: NextRequest) {
  ...
  const upstreamUrl = `${base}/api/stats/sales${params.size ? `?${params}` : ""}`;
  ...
  try {
    ...
    console.error("[BFF /api/finanzas/sales-stats]", err);
