...
const FORWARD_KEYS = ["limit", "offset", "from", "to", "tx_type", "status", "search"] as const;
...
export async function GET(req: NextRequest) {
  ...
  const upstreamUrl = `${base}/api/bank/statements${params.size ? `?${params}` : ""}`;
  ...
  try {
    ...
    console.error("[BFF /api/finanzas/banesco]", err);
