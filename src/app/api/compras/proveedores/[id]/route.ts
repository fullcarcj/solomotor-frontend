...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const up = await fetch(`${base()}/api/suppliers/${encodeURIComponent(params.id)}`, { headers: hdr(req), cache: "no-store" });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const up = await fetch(`${base()}/api/suppliers/${encodeURIComponent(params.id)}`, { method: "PATCH", headers: hdr(req), body: JSON.stringify(body), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
  } catch (e) { console.error("[BFF proveedores/[id] PATCH]", e); return NextResponse.json({ error: "Error de red." }, { status: 502 }); }
}
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const up = await fetch(`${base()}/api/suppliers/${encodeURIComponent(params.id)}`, { method: "DELETE", headers: hdr(req), cache: "no-store" });
    return NextResponse.json(await up.json().catch(() => ({})), { status: up.status });
